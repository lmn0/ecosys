...
func usage() {
	...
	fmt.Println("  gcollect -t 3600 show")
	fmt.Println("  # collect debug info for a specific request")
	fmt.Println("  gcollect -r RESTPP_2_1.1559075028795 -b 60 -d 120 -p \"error\" collect")
	fmt.Println("  # collect debug info for all components")
	fmt.Println("  gcollect -i -p \"error\" -p \"FAILED\" -s \"2019-05-22,18:00:00\" -e \"2019-05-22,19:00:00\" collect")
	fmt.Println("  # Search from log files that have been collected before")
	fmt.Println("  gcollect -i -p \"unknown\" -c admin,gpe -D -A 1 -B 2 grep")
}
...
func runCmdOnSlave() {
	prefix := "~/.gium/gcollect --slave -O " + master_dest_dir
...
func main() {
	...
	debugPtr := flag.Bool("v", false, "show debug info.")
